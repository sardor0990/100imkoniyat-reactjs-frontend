...
const Tanlovlar: React.FC = () => {
  ...
  useEffect(() => {
    const fetchCards = async () => {
      ...
      const grands = (response.data.data as Card[])?.filter(event => event.type === 'CONTESTS');
      setCards(grands);
    };
    fetchCards();
  }, []);
  ...
  const compEdit = async (card: Card) => {
    try {
      navigate(`/admin/tadbirlar/tanlovlar/qo'shish`, { state: card });
    } catch (error) {
  ...
  const Qoshish = () => {
    navigate("/admin/tadbirlar/Tanlovlar/qo'shish");
  };
    ...
    <div className="w-full">
      <div className="w-full flex pt-[10px] pl-[30px] items-center justify-between pr-[30px] pb-[10px]" style={{ background: '#FFFFFF', borderBottom: '1px solid #E5E9EB' }}>
        <div className="box-border border-b border-gray-300 bg-white">
          <span className="static w-[106px] h-[32px] flex flex-row justify-start items-center p-0 text-[24px] font-inter" style={{ fontFamily: 'Inter', fontStyle: 'normal', fontWeight: 700, }}>Tanlovlar</span>
          ...
          <div className="flex col-sm-4 flex-wrap">
            {cards.map((card) => (
              ...
              >
                ...
                <p className="w-[335px] ml-[14px] mr-[14px] mt-4 text-sm text-gray-600 font-inter" style={{ fontFamily: 'Inter', fontWeight: 400 }}>
